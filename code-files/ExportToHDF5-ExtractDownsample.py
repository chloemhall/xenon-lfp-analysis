...
# For the code to run, BrainWave5 software needs to be installed, the path for the *.dll files should be changed below accordingly
#clr.AddReference(os.path.join("C:\ProgramData\Microsoft\Windows\Start Menu\Programs\BrainWave 5", "3Brain.BrainWave.IO.dll"))
...
class writeBrw:
    def __init__(self, inputFilePath, outputFile,parameters):
        ...
        #self.QLevel = np.power(2, parameters['bitDepth']) #Probably need to adjust this now... 
...
def parameter(h5):
    ...
    parameters['maxDVolt'] = h5['/3BRecInfo/3BRecVars/MaxDigitalValue'][0]  # in eh?
    parameters['minDVolt'] = h5['/3BRecInfo/3BRecVars/MinDigitalValue'][0]  # in eh ? 
...
#def voltage_convert(data, variables):
def Digital_to_Analog(parameters): #attempt at a new V convert
    max_analog = parameters['maxUVolt']
    min_analog = parameters['minUVolt']
    max_digital = variables['maxDVolt']
    min_digital = variables['minDVolt']
    return (data - min_digital) * (max_analog - min_analog) / (max_digital - min_digital) + min_analog
...
def get_recFile_properties(path,typ):
    ...
    if typ.decode('utf8').lower() == 'bw4':
        ...
        if "Raw" in h5['Well_A1'].keys():
            ...
            parameters['maxUVolt'] = int(4125)  # in uVolt. Analog.
            parameters['minUVolt'] = int(-4125)  # in uVolt This is analog.
            parameters['maxDVolt'] = int(4095)  # in uVolt. digital.
            parameters['minDVolt'] = int(-4095)  # in uVolt This is digital. 
            ...
            parameters['minUVolt'] = int(-4125)  # in uVolt. analaog.
            parameters['maxDVolt'] = int(4095)  # in uVolt. digital.
            parameters['minDVolt'] = int(-4095)  # in uVolt This is digital. 
            #parameters['bitDepth'] = int(12)  # number of used bit of the 2 byte coding
            #parameters['qLevel'] = 2 ^ parameters['bitDepth']  # quantized levels corresponds to 2^num of bit to encode the signal
...
#    print(numReading,dur,info.get_TimeDuration().get_TotalSeconds(),dur * info.get_SamplingRate() / block_size)
    ...
    for cnk in tqdm(range(numReading), desc='Export & downsampling Progress'):
        ...
        for i in tqdm(ind_rec,desc='Extracting individual channel'):
            #ext = np.fromiter(tmp[0][i], int) # here values are converted in voltage
            ext = np.fromiter(tmp[0][int(i)], int)
            raw[:,count] = ext[:]
            count +=1
